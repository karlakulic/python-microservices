...
EXCHANGE = "events"
QUEUE = "product_service.q"
ROUTING_KEYS = ["user.created"]

def _callback(ch, method, properties, body):
    try:
        data = json.loads(body.decode("utf-8"))
        print(f"[events] {method.routing_key} -> {data}")
    except Exception as e:
        print(f"[events] error processing message: {e}")
...
def _start_consumer():
    ...
    print(f"[events] consumer started on queue={QUEUE}, keys={ROUTING_KEYS}")
    ch.start_consuming()
